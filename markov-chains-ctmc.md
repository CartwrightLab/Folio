...
Let $`P(t) = \left[p_{ij}(t)\right]`$ be an $`n \times n`$ matrix where
$`n`$ is the number of elements in $`\Sigma`$ and

``` math
p_{ij}(t) = \Pr(X(t) = j | X(0) = i)
```

Let $`\pi(0)`$ be an $`n \times 1`$ (row) vector specifying the initial
configuration of the Markov process such that

``` math
\pi_i(0) = \Pr(X(0) = i)
```

Similarly, let $`\pi(t)`$ be the vector specifying the marginal
distribution of $`X(t)`$.
...
Lemma. Homogeneous continuous-time Markov chains satisfy the
Chapman-Kolmogorov equations. For all $`\{t,s\} \ge 0`$,
...
where $`o(h)`$ uses little-o notation to describe a part of the formula
that decreases faster than $`h`$ does.
...
where $`[i = j]`$ uses Iverson bracket notation for an indicator
function.
...
Using matrix notation we can calculate $`P(t)`$ for brief segments of
time as
...
But to derive a closed-form solution we will first calculate the
derivative matrix of $`P(t)`$.
...
And given $`k`$, the total length of time $`s`$ follows an Erlang
distribution.
...
Therefore, as before, holding times following exponential distributions
with rates $`v_i`$.
...
We will next show that $`P(t) = e^{Qt}`$ by marginalizing over the
...
This is usually not possible unless the process is at equilibrium, and
if we assume equilibrium this condition reduce to

``` math
\pi_i p_{ij}(t) = \pi_j p_{ji}(t)
```

Since both sides are equal we can take the derivative with respect to
$`t`$.
...
Next let $`f_n(s)`$ be the probability density function of $`J_n`$ such
that $`\Pr(J_n \le t) = \int_0^t f_n(s)\;ds`$.
...
Since $`V = \mathrm{diag}(\{v_i\})`$, this leads to an expression for
the full matrix as
...
Let
$`p^{(1+)}_{ij}(t) = \Pr(X_t = j \text{ and at least one mutation} | X_0 = i)`$.
